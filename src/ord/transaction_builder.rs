...
impl TransactionBuilder {
        ...
        if let Some(mut deficit) = total.checked_sub(self.outputs.last().unwrap().1) {
            while deficit > Amount::ZERO {
                ...
                if benefit > deficit {
                    ...
                    tprintln!(
                        "added {value} sat input to reduce {deficit} sat deficit by {benefit} sat"
                    );
                    deficit -= benefit;
                }
            }
            ...
            if output.script_pubkey == self.recipient.script_pubkey() {
                ...
                assert!(
                    self.change_addresses
                        .iter()
                        .any(
                            |change_address| change_address.script_pubkey() == output.script_pubkey
                        ),
    ...
    fn select_cardinal_utxo(
        ...
        for utxo in &self.utxos {
            ...
            let is_closer =
                abs_diff(current_value, target_value) < abs_diff(best_value, target_value);
